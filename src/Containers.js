...
import {QuickComp,Qfx} from './Components/QuickComp.js'
            ...
            <foreignObject x={this.state.x} y={this.state.y + 24} width="160" height="24">
                <form>
                    <KeyboardEventHandler
                        handleKeys={['enter']}
                        onKeyEvent={(key, e) => this.state.enterAction()}>
                        <input
                            type="text"
                            placeholder=""
...
export class PbSVGInteractiveRectangle extends Component {
    ...
    render() {
        ...
        const x = center['x'] + adjustment['x'] + coordinates.x
        const y = center['y'] + adjustment['y'] + coordinates.y
        return (
            <g style={{userSelect: 'none',}} transform={'translate('+x+' '+y+')'}
            onMouseDown={e => {
                let cx = e.clientX
                let cy = e.clientY
                let d= true
                this.setState(
                    (prevState) => {
                        return({...prevState, origin:{ x: cx, y: cy }, dragging:d })
                    }
                )
            ...
            onMouseMove={e => {
                ...
                if (dragging) {
                    ...
                    this.setState(
                        (prevState) => {
                            return({...prevState, coordinates: {x: cx - ox, y: cy - oy} })
                        }
                    )
                }
            }}
            ...
            <Qfx/>
                <svg width="850" height="1100" viewBox="0 0 600 400">{/** onClick={this.clicked}> **/}
                    <g transform="translate(0 0)">
                        ...
                        <CalcLine x="0" y="100"/>
                        <CalcLine x="0" y="150"/>
                        <PbSVGInteractiveRectangle center = {{x:10,y:0}} adjustment = {{x:0,y:0}}/>
